...
# @see https://cloudapi.acquia.com/#POST__sites__site_dbs__db_db_copy__source__target-instance_route
...
DEPLOY_API_DB_SRC_ENV=${DEPLOY_API_DB_SRC_ENV:-}
DEPLOY_API_DB_DST_ENV=${DEPLOY_API_DB_DST_ENV:-}
DEPLOY_API_DB_SEMAPHORE_FILE=${DEPLOY_API_DB_SEMAPHORE_FILE:-}
...
[ -f "$DEPLOY_API_DB_SEMAPHORE_FILE" ] && echo "==> Skipping DB copying (found semaphore file $DEPLOY_API_DB_SEMAPHORE_FILE)" && exit 0
...
if [ "$DEPLOY_API_USER_NAME" == "" ] || [ "$DEPLOY_API_USER_PASS" == "" ] || [ "$DEPLOY_API_DB_SITE" == "" ]  || [ "$DEPLOY_API_DB_SRC_ENV" == "" ]  || [ "$DEPLOY_API_DB_DST_ENV" == "" ] ; then
  echo "==> Missing required parameter(s)" && exit 1
fi

if [ "$DEPLOY_API_DB_SRC_ENV" == "$DEPLOY_API_DB_DST_ENV" ] ; then
  echo "==> Source and destination environments cannot be the same" && exit 1
fi

[ "$DEPLOY_API_DB_SRC_ENV" != "dev" ] && [ "$DEPLOY_API_DB_SRC_ENV" != "test" ] && [ "$DEPLOY_API_DB_SRC_ENV" != "test2" ] && [ "$DEPLOY_API_DB_SRC_ENV" != "prod" ] && echo "==> Invalid source environment name: $DEPLOY_API_DB_SRC_ENV" && exit 1
[ "$DEPLOY_API_DB_DST_ENV" != "dev" ] && [ "$DEPLOY_API_DB_DST_ENV" != "test" ] && [ "$DEPLOY_API_DB_DST_ENV" != "test2" ] && [ "$DEPLOY_API_DB_DST_ENV" != "prod" ] && echo "==> Invalid destination environment name: $DEPLOY_API_DB_DST_ENV" && exit 1
...
echo "==> Copying DB from $DEPLOY_API_DB_SRC_ENV to $DEPLOY_API_DB_DST_ENV environment"
TASK_STATUS_JSON=$(curl --progress-bar -s -u $DEPLOY_API_USER_NAME:$DEPLOY_API_USER_PASS -X POST https://cloudapi.acquia.com/v1/sites/$DEPLOY_API_DB_SITE/dbs/$DEPLOY_API_DB_NAME/db-copy/$DEPLOY_API_DB_SRC_ENV/$DEPLOY_API_DB_DST_ENV.json)
...
do
  ...
  TASK_STATUS_JSON=$(curl --progress-bar -s -u $DEPLOY_API_USER_NAME:$DEPLOY_API_USER_PASS https://cloudapi.acquia.com/v1/sites/$DEPLOY_API_DB_SITE/tasks/$DEPLOYMENT_TASK_ID.json)
...
if [ $TASK_COMPLETED == 0 ] ; then
  echo "==> Unable to copy DB from $DEPLOY_API_DB_SRC_ENV to $DEPLOY_API_DB_DST_ENV environment"
  exit 1
fi

echo "==> Successfully copied DB from $DEPLOY_API_DB_SRC_ENV to $DEPLOY_API_DB_DST_ENV environment"
